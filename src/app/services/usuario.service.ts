...
export class UsuarioService {
    ...
    return this.http.get<CargarUsuario>(url).pipe(
      map( resp => {
        const usuarios = resp.usuarios.map(
          user => new Usuario(user.nombre, user.email, user.password, user.activo,  user.google, user.rol, user.uid)
        );
        return {
          total: resp.total,
          usuarios
  ...
  validarToken(): Observable<boolean> {
    ...
    return this.http.get(`${base_url}/login/renew`).pipe(
             map( (resp: any) => {
                ...
                this.usuario = new Usuario(nombre, email, '', true , google, rol, uid);
  ...
  inactivarUsuario( usuario: Usuario) {
    ...
    return this.http.put(`${base_url}/usuarios/inac/${usuario.uid}`, usuario);
  }
